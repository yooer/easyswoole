...
class Request
{
     ...
     * 获取客户端IP地址
     *
     * @param integer $type 返回类型 0 返回IP地址 1 返回IP地址数字(IPv4返回整数，IPv6返回字符串)
    ...
    /**
     * 将IPv6地址转换为数字表示
     * 返回一个字符串，因为IPv6数字太大，无法用整数表示
     *
     * @param string $ip
     * @return string
     */
    private function ipv6ToNumeric(string $ip): string
    {
        $binary = inet_pton($ip);
        if ($binary === false) {
            return '0';
    ...
    public function cookie($name = '', $default = null, $filter = '')
    {
        if (empty($this->cookie)) {
            $this->cookie = $this->esRequest->getCookieParams();
        }
